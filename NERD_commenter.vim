...
function s:AppendCommentToLine()
	...
	let cusStr = " " . strftime("%Y-%m-%d") . " add by yy"
    ...
    execute ":normal! " . insOrApp . (isLineEmpty ? '' : ' ') . left . cusStr . right . " "
	...
	let cusStrlen = strlen(cusStr)
    let leftMoveAmount = lenRight + cusStrlen
                ...
                if s:CanPlaceCursor(currentLine, lSide)
                    ...
                    if s:Multipart()
                        ...
                        if firstLeftDelim != -1 && lastRightDelim != -1
                            ...
                            let theLine = strpart(theLine, 0, firstLeftDelim+strlen(s:Left())) . theLineWithPlaceHolders . strpart(theLine, lastRightDelim) . ' ' .  strftime("%Y-%m-%d") . ' add by yy'
                        endif
                    endif
                endif
            endif
